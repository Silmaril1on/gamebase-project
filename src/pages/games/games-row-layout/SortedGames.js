...
function SortedGames({ data }) {
  ...
  return (
    <>
      <div className="flex justify-start flex-row space-x-3 px-2">
        {data.map((item) => {
          const { name, price, platforms, image, game, free } = item;
          return (
            <div className=" w-24 h-48 md:w-40 mb-4 bg-black md:h-80 relative cursor-pointer group outline outline-offset-2 outline-none hover:cream-outline duration-300">
              ...
              <article className="font-secondary py-1 flex-col child:text-start">
                ...
                <h1 className="text-xs md:text-sm font-bold uppercase">
                  {name}
                </h1>
                {price === null ? (
                  ""
